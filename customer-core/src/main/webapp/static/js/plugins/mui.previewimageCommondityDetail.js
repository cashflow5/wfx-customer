(function(M, window) {
	...
	proto.init = function() {
		...
		this.element = el;
		this.scroller = this.element.querySelector(M.classSelector('.slider-group'));
		this.indicator = this.element.querySelector(M.classSelector('.preview-indicator'));
		this.loader = this.element.querySelector(M.classSelector('.preview-loading'));
		if (options.footer) {
			this.element.querySelector(M.classSelector('.preview-footer')).classList.remove(M.className('hidden'));
		}
		this.addImages();
	};
	proto.initEvent = function() {
		var self = this;
		var getFirstImg=null;//详情页面添加代码
		M(document.body).on('tap', 'img[data-preview-src]', function() {
			getFirstImg=this;//详情页面添加代码
		...
		/*详情页面添加代码 start*/
		M(document.body).on('tap', '.img-goods-100', function() {
			var _this=null;
			if(getFirstImg!=null){
				_this=getFirstImg;
			}else{
				_this=$('img[data-preview-src]').get(0);
		...
		var laterCloseEvent = function() {
			!laterClose && (laterClose = M.later(function() {
		...
		this.element.addEventListener('webkitAnimationEnd', function() {
			if (self.element.classList.contains(M.className('preview-out'))) { //close
				self.element.style.display = 'none';
				self.element.classList.remove(M.className('preview-out'));
				laterClose = null;
			} else { //open
	...
	proto._imgTransitionEnd = function(e) {
		var img = e.target;
		img.classList.remove(M.className('transitioning'));
	...
	proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
		var itemEl = this.scroller.querySelector(M.classSelector('.slider-item:nth-child(' + (index + 1) + ')'));
		...
		if (!itemData.loaded && imgEl.getAttribute('data-preview-lazyload')) {
			var self = this;
			self.loader.classList.add(M.className('active'));
			//移动位置动画
			...
			this.loadImage(imgEl, function() {
				...
				imgEl.classList.add(M.className('transitioning'));
				imgEl.addEventListener('webkitTransitionEnd', self._imgTransitionEnd.bind(self));
				imgEl.setAttribute('style', '');
				imgEl.offsetHeight;
				self.loader.classList.remove(M.className('active'));
			});
		} else {
	...
	proto._initZoom = function(zoomWrapperEl, zoomerWidth, zoomerHeight) {
		...
		var zoomEl = zoomWrapperEl.querySelector(M.classSelector('.zoom'));
	...
	proto.refresh = function(index, groupArray) {
		...
		this.element.classList.add(M.className('preview-in'));
		this.lastIndex = currentIndex;
		this.element.offsetHeight;
		M(this.element).slider().gotoItem(currentIndex, 0);
	...
	proto.open = function(index, group) {
		if (this.element.classList.contains(M.className('preview-in'))) {
			return;
		}
	...
	proto.close = function(index, group) {
		this.element.classList.remove(M.className('preview-in'));
		this.element.classList.add(M.className('preview-out'));
		var itemEl = this.scroller.querySelector(M.classSelector('.slider-item:nth-child(' + (this.lastIndex + 1) + ')'));
		var imgEl = itemEl.querySelector('img');
		if (imgEl) {
			imgEl.classList.add(M.className('transitioning'));
			...
			if (sTop > window.innerHeight || sLeft > window.innerWidth || sTop < 0 || sLeft < 0) { //out viewport
				...
				if (this.options.zoom) {
					M(imgEl.parentNode.parentNode).zoom().toggleZoom(0);
				}
				imgEl.style.webkitTransitionDuration = '0.5s';
				imgEl.style.webkitTransform = 'translate3d(' + posi.x + 'px,' + posi.y + 'px,0) scale(' + itemData.sScale + ')';
			}
		}
		var zoomers = this.element.querySelectorAll(M.classSelector('.zoom-wrapper'));
		for (var i = 0, len = zoomers.length; i < len; i++) {
			M(zoomers[i]).zoom().destory();
		}
		//		M(this.element).slider().destory();
		//		this.empty();
	};
	proto.isShown = function() {
		return this.element.classList.contains(M.className('preview-in'));
	};

	var previewImageApi = null;
	M.previewImage = function(options) {
		if (!previewImageApi) {
			previewImageApi = new PreviewImage(options);
		}
		return previewImageApi;
	};
	M.getPreviewImage = function() {
		return previewImageApi;
	}
